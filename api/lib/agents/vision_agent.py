...
def run_full_vision_analysis(
    ...
    *,
    ...
    def _one(idx: int, image_path: Path) -> tuple[int, dict[str, Any]]:
        ...
        try:
            ...
            return idx, {"error": str(exc), "file": image_path.name}

    if max_workers is not None and max_workers >= 2 and len(images) > 1:
        ordered: list[dict[str, Any] | None] = [None] * len(images)
        with ThreadPoolExecutor(max_workers=max_workers) as pool:
            futures = {pool.submit(_one, i, path): i for i, path in enumerate(images)}
            for fut in as_completed(futures):
                idx, out = fut.result()
                ordered[idx] = out
        return [o if o is not None else {"error": "internal_order", "file": "?"} for o in ordered]

    results: list[dict[str, Any]] = []
    for i, image_path in enumerate(images):
        _, out = _one(i, image_path)
        results.append(out)
    return results
