...
`app/config.py` (pydantic-settings): base de datos, Redis, CORS, secretos JWT, claves de OpenAI cuando apliquen. En Docker Compose las URLs apuntan a los servicios `postgres` y `redis`.
...
`docker compose up --build` levanta servicios integrados; el frontend se sirve compilado (p. ej. Nginx) con proxy a la API.
