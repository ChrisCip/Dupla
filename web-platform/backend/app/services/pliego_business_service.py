...
from app.services.clash_service import extract_clash_report
...
def _clash_risks_text(clash_report: dict[str, Any] | None) -> str:
    if not clash_report:
        return ""
    clashes = clash_report.get("clashes") or []
    if not isinstance(clashes, list) or not clashes:
        return ""
    lines: list[str] = ["Riesgos de coordinación detectados (análisis automático):"]
    priority_rank = {"critical": 0, "high": 1, "warning": 2, "info": 3}
    sorted_clashes = sorted(
        [c for c in clashes if isinstance(c, dict)],
        key=lambda c: priority_rank.get(str(c.get("priority") or "info"), 9),
    )
    for c in sorted_clashes[:8]:
        pri = str(c.get("priority") or "info").upper()
        title = str(c.get("title") or "Conflicto")
        desc = str(c.get("description") or "")[:300]
        lines.append(f"- [{pri}] {title}: {desc}")
    return "\n".join(lines)


def _merge_risks_sections(
    findings: list[ProjectTechnicalFinding],
    clash_report: dict[str, Any] | None,
) -> str:
    base = _findings_risks_text(findings)
    clash = _clash_risks_text(clash_report)
    if clash and base.startswith("Sin riesgos"):
        return clash
    if clash:
        return f"{base}\n\n{clash}"
    return base
...
class PliegoBusinessService:
    ...
    async def _clash_report_for_project(self, project_id: UUID) -> dict[str, Any] | None:
        q = (
    ...
    def build_sections_dict(
        ...
        findings: list[ProjectTechnicalFinding],
        clash_report: dict[str, Any] | None = None,
    ) -> dict[str, str]:
        ...
        return {
            ...
            SECTION_RISKS: _merge_risks_sections(findings, clash_report),
        }
    ...
    async def build_draft_block(self, project: Project) -> dict[str, Any]:
        ...
        clash_report = await self._clash_report_for_project(project.id)
        sections = self.build_sections_dict(project, files, findings, clash_report=clash_report)
