...
from app.domain.coordination_profile import apply_inferred_profile_if_empty, infer_profile_from_project_fields
...
class ProjectLifecycleService:
    ...
    async def update_project_meta(self, user: User, project_uuid: UUID, patch: dict[str, Any]) -> Project:
        ...
        if "name" in patch and patch["name"] is not None:
            ...
            if not project.coordination_profile:
                inferred = infer_profile_from_project_fields(name=project.name, project_code=project.project_code)
                if inferred:
                    project.coordination_profile = inferred
                    payload["coordination_profile_inferred"] = inferred
        ...
        if "project_code" in patch:
            ...
            if not project.coordination_profile:
                ...
                if inferred:
                    project.coordination_profile = inferred
                    payload["coordination_profile_inferred"] = inferred
        if "coordination_profile" in patch:
            cp = str(patch["coordination_profile"]).strip() if patch["coordination_profile"] is not None else ""
            cp = cp or None
            payload["coordination_profile"] = {"from": project.coordination_profile, "to": cp}
            project.coordination_profile = cp
