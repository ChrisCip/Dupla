"""
Dupla multi-discipline runner for the NASAS 9 project.
...
Usage:
    python dupla_run_nasas.py
    python dupla_run_nasas.py --only arquitectura
    python dupla_run_nasas.py --resume
"""
...
# ========= CONFIG =========
PROJECT_NAME = "Residencial NASAS 9"
PROJECT_ID = "nasas_9"

DISCIPLINES: dict[str, dict[str, str]] = {
    "arquitectura": {
        "pdf": r"documentos_nasas/PLANOS ARQ.-LAS NASAS 09-20260320.pdf",
        "dwg": r"documentos_nasas/PLANOS ARQ.-LAS NASAS 09-20260320.dwg",
    },
    "estructura": {
        "pdf": r"documentos_nasas/PLANOS ESTRUCTURALES-LAS NASAS 09.pdf",
        "dwgs": [
            r"documentos_nasas/20.03.2026 LAS NASAS 09 ES-01-DETALLES GENERALES.dwg",
            r"documentos_nasas/20.03.2026 LAS NASAS 09 ES-02@04-CIMIENTOS-LAS NASAS-DESSANGLES.dwg",
            r"documentos_nasas/20.03.2026 LAS NASAS 09 ES-05@11- PLANTAS ENTREPISO-LAS NASAS-DESSANGLES.dwg",
            r"documentos_nasas/20.03.2026 LAS NASAS 09 ES-12-DETALLES DE ENCOFRADO.dwg"
        ],
    },
    "electrico": {
        "pdf": r"documentos_nasas/11.03.2026 LAS NASAS 09 - PLANOS ELECTRICOS.pdf",
        "dwg": r"documentos_nasas/20.03.2026 LAS NASAS 09-PLANOS ELECTRICOS .dwg",
    },
    "sanitario": {
        "pdf": r"documentos_nasas/",
        "dwg": r"documentos_nasas/15.04.2026 LAS NASAS 09 HS-AGUA POTABLE.dwg",
    },
}
...
OUTPUTS_DIR = r"output/outputs_nasas"
...
def run_preflight(disciplines: dict[str, dict[str, str]], bc3_path: str) -> list[str]:
    ...
    for disc_id, files in disciplines.items():
        ...
        dwg_paths = list(files.get("dwgs", []))
        if files.get("dwg"):
            dwg_paths.append(files["dwg"])
        for dp in dwg_paths:
            dwg = Path(dp)
            if not dwg.exists():
...
def process_discipline(
    ...
    *,
    ...
    dwg_paths_str = list(files.get("dwgs", []))
    if files.get("dwg"):
        dwg_paths_str.append(files["dwg"])
        
    if dwg_paths_str:
        valid_dwgs = [p for p in dwg_paths_str if Path(p).exists()]
        if valid_dwgs:
            all_raw_data = []
            try:
                token = get_aps_token()
                ...
                for idx, dwg_p in enumerate(valid_dwgs):
                    logger.info("[%s] APS extraction (%d/%d): %s", disc_id, idx+1, len(valid_dwgs), Path(dwg_p).name)
                    unique_suffix = datetime.now().strftime("%Y%m%d_%H%M%S")
                    uploaded_name = upload_file_to_bucket(
                        token, bucket_name, dwg_p,
                        unique_suffix=unique_suffix,
                    ) or ""

                    if uploaded_name:
                        logger.info("[%s] DWG uploaded as '%s'", disc_id, uploaded_name)
                        raw_data = extract_dwg_data(
                            token, bucket_name, uploaded_name,
                        ...
                        all_raw_data.append({"dwg": dwg_p, "data": raw_data})
                        
                        temp_raw_json = disc_dir / f"autodesk_raw_{idx}.json"
                ...
                logger.info("[%s] Total CAD facts combined: %d layers", disc_id, len(cad_facts.get("cad_facts", {}).get("layers", {})))
            except Exception as exc:
                logger.warning("[%s] APS extraction failed, continuing with PDF only: %s", disc_id, exc)
        else:
            logger.info("[%s] No valid DWGs found, using PDF only", disc_id)
    else:
    ...
    if dwg_paths_str and not building_block:
        building_block, level_id = parse_location_from_filename(Path(dwg_paths_str[0]).name)
