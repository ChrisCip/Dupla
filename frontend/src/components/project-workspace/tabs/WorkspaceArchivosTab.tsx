...
export function WorkspaceArchivosTab({ projectUuid, token, flowMsg }: WorkspaceArchivosTabProps) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const filesQ =
        folderUuid === null ? '' : `?folder_uuid=${encodeURIComponent(folderUuid)}`
      const [fr, fe] = await Promise.all([
        apiFetch(`/api/projects/${projectUuid}/file-folders${fq}`, { token }),
        apiFetch(`/api/projects/${projectUuid}/files${filesQ}`, { token }),
      ])
  ...
  const filteredFolders = useMemo(() => {
    ...
    if (!q) return folders
    return folders.filter((fo) => fo.name.toLowerCase().includes(q))
  }, [folders, filterSearch])

  const filteredFiles = useMemo(() => {
    return files.filter((f) => {
      if (filterDiscipline && (f.discipline ?? '') !== filterDiscipline) return false
      const q = filterSearch.trim().toLowerCase()
      if (!q) return true
  ...
  }, [files, filterDiscipline, filterSearch])

  const hasActiveFilters = Boolean(filterDiscipline || filterSearch.trim())
  const gridIsEmpty = filteredFolders.length === 0 && filteredFiles.length === 0
    ...
    <Card className="space-y-4 p-6">
      ...
      >
        {busy ? (
          ...
          <p className="py-8 text-center text-sm text-muted">
            Ningún resultado con los filtros actuales. Ajusta disciplina o búsqueda, o usa Limpiar.
          </p>
        ) : (
